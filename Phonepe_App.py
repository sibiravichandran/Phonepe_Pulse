...
def display_navigation():
    ...
    menu_title = "",  
    options = ["Home","About","Overview of Insights","Analyze Insights","Discoveries","Contact"],
    icons =["house","bar-chart","toggles","search","list-task", 'at'],
...
def About():
    ...
    st.subheader(":violet[The Indian digital payments story has truly captured the world's imagination. From the largest towns to the remotest villages, there is a payments revolution being driven by the penetration of mobile phones and data.]")
    st.subheader(":violet[When PhonePe started 5 years back, we were constantly looking for definitive data sources on digital payments in India. Some of the questions we were seeking answers to were - How are consumers truly using digital payments? What are the top cases? Are kiranas across Tier 2 and 3 getting a facelift with the penetration of QR codes?]")               
    st.subheader(":violet[This year as we became India's largest digital payments platform with 46% UPI market share, we decided to demystify the what, why and how of digital payments in India.]")
    st.subheader(":violet[This year, as we crossed 2000 Cr. transactions and 30 Crore registered users, we thought as India's largest digital payments platform with 46% UPI market share, we have a ring-side view of how India sends, spends, manages and grows its money. So it was time to demystify and share the what, why and how of digital payments in India.]")
    st.subheader(":violet[PhonePe Pulse is your window to the world of how India transacts with interesting trends, deep insights and in-depth analysis based on our data put together by the PhonePe team.]")
            ...
            if menu=="User":
                ...
                if select_option == 'Aggregated':
                    ...
                    #st.dataframe(df_agg)
                        ...
                        india_map_user = df_agg[['State','Reg_Users','App_Opens','Count']].groupby(['State']).sum().reset_index()
...
def Fun_Facts():
    ...
    with st.sidebar:
        ...
        menu_title = "PhonePe Pulse India Fun Facts",  
        options = [
